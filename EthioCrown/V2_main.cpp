...
void generateCylinder(float radius, float height, int sectors, GLfloat* vertices) {
    ...
    for (int i = 0; i <= sectors; ++i) {
        ...
        // Normal for bottom vertex
        vertices[index++] = cos(angle);          // Normal X
        vertices[index++] = 0.0f;                // Normal Y
        vertices[index++] = sin(angle);          // Normal Z

        // Top circle vertex
        vertices[index++] = radius * cos(angle);  // X
        vertices[index++] = height / 2;           // Y
        vertices[index++] = radius * sin(angle);  // Z
        // Normal for top vertex
        vertices[index++] = cos(angle);          // Normal X
        vertices[index++] = 0.0f;                // Normal Y
        vertices[index++] = sin(angle);          // Normal Z
    }
...
int main() {
   ...
   // glGenBuffers(3, VBO);
    ...
    glBufferData(GL_ARRAY_BUFFER, vertexCount * 6 * sizeof(GLfloat), vertices, GL_STATIC_DRAW);
   ...
   //  glDrawArrays(GL_LINES, 0, vertexCount);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        shader.setVec3("viewPos", glm::vec3(0.0f, 0.0f, 3.0f)); // View position
